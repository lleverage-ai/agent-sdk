...
export function createObservabilityPreset(
  ...
  if (enableHooks) {
    ...
    if (enableLogging && preset.logger) {
      ...
      // Team event logging hook factory
      const teamLogger = (eventName: string): HookCallback =>
        async (input) => {
          logger.info(`[${name}] [Team] ${eventName}`, { ...input });
          return undefined;
        };
      ...
      hooks = {
        ...
        TeamMessageReceived: [teamLogger("MessageReceived")],
        TeamTaskAssigned: [teamLogger("TaskAssigned")],
        TeamTaskCompleted: [teamLogger("TaskCompleted")],
        TeamTaskCreated: [teamLogger("TaskCreated")],
        TeamTaskFailed: [teamLogger("TaskFailed")],
        TeamTeammateSpawned: [teamLogger("TeammateSpawned")],
        TeamTeammateCrashed: [teamLogger("TeammateCrashed")],
        TeamShutdownRequested: [teamLogger("ShutdownRequested")],
        TeamShutdownComplete: [teamLogger("ShutdownComplete")],
        TeamPlanSubmitted: [teamLogger("PlanSubmitted")],
        TeamPlanApproved: [teamLogger("PlanApproved")],
        TeamPlanRejected: [teamLogger("PlanRejected")],
      };
    }

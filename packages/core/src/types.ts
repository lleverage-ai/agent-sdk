...
export interface AgentOptions {
   ...
   *
   * When set, this agent is part of a team managed by `@lleverage-ai/agent-teams`.
   * The core SDK does not use this field directly — it is read by the agent-teams
   * package to configure mailbox, task-queue, and inter-agent messaging.
   *
  ...
  teamContext?: {
    ...
    agentId: string;
    /** Role of this agent: lead orchestrates, teammate executes */
    role: "lead" | "teammate";
    /** Base directory for team coordination files */
    baseDir: string;
  };
}
...
export interface AgentPlugin {
  ...
  /** Hook registrations provided by this plugin */
  hooks?: HookRegistration;
}
...
export type HookEvent =
  ...
  // Team lifecycle (fired by @lleverage-ai/agent-teams, never by core SDK)
  | "TeamMessageReceived"
  | "TeamTaskAssigned"
  | "TeamTaskCompleted"
  | "TeamTaskCreated"
...
export interface HookRegistration {
  ...
  /**
   * Team lifecycle hooks (fired by @lleverage-ai/agent-teams).
   * These are never fired by the core SDK — the agent-teams package
   * invokes them when team events occur.
   */
  TeamMessageReceived?: HookCallback[];
  TeamTaskAssigned?: HookCallback[];
  TeamTaskCompleted?: HookCallback[];
  TeamTaskCreated?: HookCallback[];
  TeamTaskFailed?: HookCallback[];
  TeamTeammateSpawned?: HookCallback[];
  TeamTeammateCrashed?: HookCallback[];
  TeamShutdownRequested?: HookCallback[];
  TeamShutdownComplete?: HookCallback[];
  TeamPlanSubmitted?: HookCallback[];
  TeamPlanApproved?: HookCallback[];
  TeamPlanRejected?: HookCallback[];
}

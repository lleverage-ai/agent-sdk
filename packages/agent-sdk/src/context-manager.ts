...
export interface CompactionResult {
  ...
  /**
   * Token usage of the summary generation, when reported by a configured
   * {@link ContextManagerOptions.summarizer}. Absent for agent-generated summaries.
   */
  summaryUsage?: SummaryUsage;

  /**
   * Wall-clock milliseconds from entry to `compact()` through construction of
   * this result, when a summary was generated. Covers counting and retention
   * selection as well as the summary call. Absent on the no-op result.
   */
...
export interface ContextManagerOptions {
  ...
  /**
   * Executes summary generation in place of `agent.generate()`.
   *
   * When set, `compact()` never calls the agent passed to it; every summary
   * request (rollup, structured and tiered) goes through this executor with
   * the exact messages the built-in path would send. Use it to run summaries
   * on an isolated model or agent with no tools or hooks, to bound them with
   * a deadline or the run's abort signal, and to report their token usage.
   * A rejection fails the compaction like a failed model call would. Empty
   * text is accepted exactly as an empty agent-generated summary is; reject
   * it in the executor if it must fail the compaction.
   *
   * @defaultValue undefined (summaries are generated by the agent passed to `compact()`)
   */
  summarizer?: SummaryExecutor;
  ...
  /**
   * Awaited after a compaction result is generated and `onCompact` has run,
   * before `compact()` resolves. Use it to write the compacted transcript
   * durably so that whoever awaits the compaction also awaits the write.
   *
   * The in-memory result is already valid when this runs. A rejection is
   * contained: `compact()` still resolves with the result and the compaction
   * circuit records a success, so a failed write costs a recompute later
   * rather than the active generation. Log and classify outcomes inside the
   * callback; the context manager does not interpret them.
   *
   * @defaultValue undefined
   */
  commitCompaction?: (result: CompactionResult) => Promise<void> | void;
}

/**
 * A summary generation request handed to {@link ContextManagerOptions.summarizer}.
 *
 * @category Context
 */
export interface SummaryRequest {
  /** The system prompt and user content the built-in path would send. */
  messages: ModelMessage[];
  /** Output token limit from the summarization config. */
  maxTokens: number;
  /** Reason the enclosing compaction was triggered. */
  trigger: CompactionTrigger;
...
/**
 * Token usage reported for a summary generation.
 *
 * @category Context
 */
...
/**
 * The outcome of a {@link SummaryExecutor} call.
 *
 * @category Context
 */
...
/**
 * Generates a compaction summary outside the agent being compacted.
 *
 * @category Context
 */
export type SummaryExecutor = (request: SummaryRequest) => Promise<SummaryResponse>;
...
export function createContextManager(options: ContextManagerOptions): ContextManager {
  ...
  const { maxTokens, onBudgetUpdate, onCompact, commitCompaction, summarizer } = options;
  ...
  const compact = async (
    ...
    /**
     * Run one summary generation through the configured executor, or the
     * agent when none is configured. The messages are identical either way.
     */
    ...
    try {
      ...
      const result: CompactionResult = {
        ...
        summaryTier,
        ...(summaryUsage ? { summaryUsage } : {}),
        summaryDurationMs: performance.now() - startedAt,
      };
      ...
      if (commitCompaction) {
        // Awaited so callers that await compaction also await the durable
        // write; contained so a failed write never fails a valid compaction.
        try {
          await commitCompaction(result);
        } catch {
          // The host logs and classifies its own write outcomes.
        }
      }

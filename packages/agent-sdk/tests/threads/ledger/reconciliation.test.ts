...
import {
  ...
  listStaleRuns,
  recoverAllStaleRuns,
} from "../../../src/threads/ledger/reconciliation.js";
...
describe("Reconciliation", () => {
  ...
  it("recoverAllStaleRuns recovers all stale runs", async () => {
    ...
    const { succeeded, failed } = await recoverAllStaleRuns(store, "fail", { olderThanMs: 0 });
  ...
  it("recoverAllStaleRuns with cancel action", async () => {
    ...
    const { succeeded, failed } = await recoverAllStaleRuns(store, "cancel", { olderThanMs: 0 });
    expect(succeeded).toHaveLength(1);
    expect(succeeded[0]!.newStatus).toBe("cancelled");
    expect(failed).toHaveLength(0);
  ...
  it("recoverAllStaleRuns returns empty when nothing is stale", async () => {
    const store = new InMemoryLedgerStore();
    const { succeeded, failed } = await recoverAllStaleRuns(store, "fail", { olderThanMs: 60_000 });
    expect(succeeded).toEqual([]);
    expect(failed).toEqual([]);
  });

  it("recoverAllStaleRuns reports both successes and failures", async () => {
    const store = new InMemoryLedgerStore();
    const r1 = await store.beginRun({ threadId: "t1" });
    const r2 = await store.beginRun({ threadId: "t1" });
    ...
    const { succeeded, failed } = await recoverAllStaleRuns(store, "fail", { olderThanMs: 0 });
    expect(succeeded).toHaveLength(1);
    expect(succeeded[0]!.runId).toBe(r2.runId);
    expect(failed).toHaveLength(1);
    expect(failed[0]!.runId).toBe(r1.runId);
